...
local startClaimshieldProcess = function(realMob, engager)
    ...
    if not dynamicMob then
        -- Fallback: unhide real mob and abort. CS_Triggered stays latched so that the
        -- re-engage below does not retry a spawn that just failed.
...
endClaimshieldProcess = function(realMob)
    ...
    -- CS_Triggered is deliberately left set. Unhiding re-awards claim, which makes the NM
    -- engage the winner and fire ENGAGE again, still inside engageWindowSeconds.
...
claimRecoveryCheck = function(mob)
    ...
    if winner and mob:isSpawned() then
        -- updateClaim reassigns the owner unconditionally; it does not defer to an existing
        -- claim. That is safe only because claim cannot lapse within claimRecoveryDelay:
        -- deaggro needs 25s + map.MOB_ADDITIONAL_TIME_TO_DEAGGRO. Do not widen the delay
        -- without adding a hasClaim() check first.
...
local spawnListenerFunc = function(mob)
    ...
    mob:setLocalVar('CS_Triggered', 0)
end
...
local engageListenerFunc = function(mob, target)
    -- At most one claimshield per spawn. ENGAGE fires whenever the NM enters combat,
    -- including when endClaimshieldProcess re-awards claim to the lottery winner, so a
    -- flag cleared at the end of the process would let the shield restart.
    -- Only spawnListenerFunc clears this.
    if mob:getLocalVar('CS_Triggered') == 1 then
        return
    end
    ...
    mob:setLocalVar('CS_Triggered', 1)
